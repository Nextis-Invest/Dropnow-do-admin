...
export default async function ExternalUsersPage() {
  ...
  const recentUsers = await prisma.externalUser.findMany({
    where: {
      lastConnected: {
        not: null,
      },
          ...
          <div className="@container/main flex flex-1 flex-col gap-2">
            <div className="flex flex-col gap-4 py-4 md:gap-6 md:py-6">
              <div className="px-4 lg:px-6">
                <h1 className="text-2xl font-bold tracking-tight">External Users</h1>
                      ...
                      {recentUsers.length > 0 ? (
                        <div className="space-y-4">
                          {recentUsers.map((user) => (
                            <div key={user.id} className="flex items-start space-x-4 border-b pb-4 last:border-0">
                              ...
                              <div className="flex-1 space-y-1">
                                ...
                                <div className="flex items-center text-xs text-muted-foreground">
                                  ...
                                  Connected {user.lastConnected
                                    ? formatDistanceToNow(user.lastConnected, { addSuffix: true })
                                    : "never"}
                                </div>
                                {user.mobileDevices.length > 0 && (
                                  <div className="text-xs text-muted-foreground">
                                    <span className="font-medium">Device:</span>{" "}
                                    {user.mobileDevices[0].deviceName || user.mobileDevices[0].deviceModel || "Unknown device"}
                                  </div>
                                )}
                              </div>
                            </div>
